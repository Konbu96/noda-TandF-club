...
import { deleteMenu, getMenu, saveMenu } from '@/member-app/menu/menuService';
...
export default function MenuDetailPage() {
  ...
  return (
    <div className="px-4 py-6 space-y-4">
      <Link href="/member/menu" className="text-sm text-blue-900">← 一覧に戻る</Link>
        ...
        <>
          ...
          {canEditMenu && !isEditing && (
            <button
              ...
              disabled={deleting}
              className="w-full text-sm border border-red-200 text-red-500 py-2 rounded-xl disabled:opacity-50"
            >
              {deleting ? '削除中...' : 'このメニューを削除'}
            </button>
          )}
        </>
      )}
